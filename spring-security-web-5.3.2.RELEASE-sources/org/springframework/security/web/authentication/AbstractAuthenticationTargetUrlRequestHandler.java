...
public abstract class AbstractAuthenticationTargetUrlRequestHandler {
	 ...
	 *提供默认的目标Url，如果在会话中未找到已保存的请求，或者alwaysUseDefaultTargetUrl属性设置为true，则将使用该默认目标。 如果未设置，则默认为/。 它将被视为相对于Web应用程序的上下文路径，并且应包含前导/。 或者，包含方案名称（例如“ http：//”或“ https：//”）作为前缀将表示完全限定的URL，并且也支持此名称。
	 ...
	 * 如果设置了此属性，将使用该名称的参数检查当前请求，并使用该参数作为目标URL（如果存在）的值。
	 * @param targetUrlParameter the name of the parameter containing the encoded target
	 * URL. Defaults to null.包含编码的目标URL的参数的名称。 默认为空。
	 */
