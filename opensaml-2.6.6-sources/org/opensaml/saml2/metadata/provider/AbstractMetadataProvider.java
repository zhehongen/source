...
public abstract class AbstractMetadataProvider extends BaseMetadataProvider {
    ...
    private Map<String, EntityDescriptor> indexedDescriptors;//说明：什么id     entityID
    ...
    /**获取提供者当前持有的元数据。 此方法不应检查提供者是否已初始化，元数据是否有效等。所有这些均由此方法的调用者完成。
    ...
    protected EntitiesDescriptor doGetEntitiesDescriptor(String name) throws MetadataProviderException {
        ...
        if (metadata instanceof EntitiesDescriptor) {//说明：怎么可能
    ...
    /** {@inheritDoc} */
    public EntityDescriptor getEntityDescriptor(String entityID) throws MetadataProviderException {//看过了
        if (!isInitialized()) {
            throw new MetadataProviderException("Metadata provider has not been initialized");
        }
     ...
     */
    protected EntityDescriptor doGetEntityDescriptor(String entityID) throws MetadataProviderException {//看过了
        XMLObject metadata = doGetMetadata();
        if (metadata == null) {
    ...
    /** {@inheritDoc} */
    public List<RoleDescriptor> getRole(String entityID, QName roleName) throws MetadataProviderException {//说明：list
        if (!isInitialized()) {
            throw new MetadataProviderException("Metadata provider has not been initialized");
        }
     ...
     */
    protected List<RoleDescriptor> doGetRole(String entityID, QName roleName) throws MetadataProviderException {//看过了
    ...
    /**从给定的流中解组元数据。 通过此方法关闭流，并且返回的元数据释放其DOM表示形式。
    ...
    /**从缓存的元数据中获取具有给定ID的EntityDescriptor。
     ...
     */
    protected EntityDescriptor getEntityDescriptorById(String entityID, XMLObject metadata) {//看过了
        EntityDescriptor descriptor = null;

        log.debug("Searching for entity descriptor with an entity ID of {}", entityID);
        if (entityID != null && indexedDescriptors.containsKey(entityID)) {//说明：缓存
        ...
        if (metadata != null) {
            if (metadata instanceof EntityDescriptor) {//一般般
     ...
     */
    protected EntityDescriptor getEntityDescriptorById(String entityID, EntitiesDescriptor descriptor) {//看过了
     ...
     */
    protected EntitiesDescriptor getEntitiesDescriptorByName(String name, EntitiesDescriptor rootDescriptor) {//看过了
        EntitiesDescriptor descriptor = null;
    ...
    /**返回给定的描述符是否有效。 如果不需要有效的元数据，则此方法始终返回true。
