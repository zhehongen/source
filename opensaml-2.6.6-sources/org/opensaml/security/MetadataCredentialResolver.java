...
/**能够从SAML 2元数据解析凭据的凭据解析器；传递给resolve（CriteriaSet）和resolveSingle（CriteriaSet）的CriteriaSet实例必须至少包含2个条件：EntityIDCriteria和MetadataCriteria。 EntityIDCriteria.getEntityID（）和MetadataCriteria.getRole（）的值是必需的。如果未提供通过MetadataCriteria.getProtocol（）获得的协议值，则无论协议支持如何，都将从所有匹配角色中解析凭据。UsageCriteria的规范是可选的。如果条件集中不存在使用条件，则有效值UsageType.UNSPECIFIED将用于凭据解析。此凭据解析器将已解析的凭据缓存在内存敏感的缓存中。如果元数据提供者是ObservableMetadataProvider，则当基础元数据发生更改时，此解析器还将清除其缓存。
...
//说明：从元数据中解析凭据
    ...
    protected Iterable<Credential> resolveFromSource(CriteriaSet criteriaSet) throws SecurityException {
        ...
        log.debug("Forcing on-demand metadata provider refresh if necessary");//说明：必要时强制按需元数据提供程序刷新
        try {
    ...
    protected Collection<Credential> retrieveFromMetadata(String entityID, QName role, String protocol, UsageType usage)
            throws SecurityException {//说明：从元数据文档里面居然解析出来了凭据。凭据和和证书啥关系
        ...
        for (RoleDescriptor roleDescriptor : roleDescriptors) {//说明：IDPSSODescriptor和SPSSODescriptor 也是RoleDescriptor
            ...
            for (KeyDescriptor keyDescriptor : keyDescriptors) {
                ...
                if (matchUsage(mdUsage, usage)) {
                    if (keyDescriptor.getKeyInfo() != null) {
                        ...
                        Iterable<Credential> creds = getKeyInfoCredentialResolver().resolve(critSet);//说明：用系统提供的resolver从keyInfo中提取出来了凭据
                        if(credentials == null){//说明：怎么可能为空？奶奶的
                            continue;
                        }
     ...
     */
    protected boolean matchUsage(UsageType metadataUsage, UsageType criteriaUsage) {//看过了
    ...
    protected List<RoleDescriptor> getRoleDescriptors(String entityID, QName role, String protocol)
        ...
        try {
            ...
            if (DatatypeHelper.isEmpty(protocol)) {
                ...
                RoleDescriptor roleDescriptor = metadata.getRole(entityID, role, protocol);//看过了
     ...
     */
    protected void cacheCredentials(MetadataCacheKey cacheKey, Collection<Credential> credentials) {//看过了
