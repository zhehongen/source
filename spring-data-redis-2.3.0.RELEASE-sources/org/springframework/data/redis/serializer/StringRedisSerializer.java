...
/**简单的字符串到字节[]（向后）序列化程序。使用指定的字符集（默认为UTF-8）将字符串转换为字节，反之亦然。当与Redis的交互主要通过字符串进行时非常有用。不执行任何空转换，因为空字符串是有效的键/值。
...
public class StringRedisSerializer implements RedisSerializer<String> {
	...
	/**StringRedisSerializer使用7位ASCII，又称ISO646-US，又称Unicode字符集的基本拉丁块。
	...
	/**StringRedisSerializer使用8位UCS转换格式。
