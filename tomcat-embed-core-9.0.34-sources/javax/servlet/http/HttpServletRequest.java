...
public interface HttpServletRequest extends ServletRequest {
     ...
     * 返回请求URI中指示请求上下文的部分。 上下文路径总是在请求URI中排在第一位。
     * 路径以“ /”字符开头，但不以“ /”字符结尾。 对于默认（根）上下文中的servlet，此方法返回“”。
     * 容器不解码此字符串。
     *
     * 返回值：
     * 一个字符串，它指定请求URI中指示请求上下文的部分
     */
     ...
     * 为确保正确维护会话，必须在提交响应之前调用此方法。 如果容器使用cookie来维护会话的完整性，
     * 并且在提交响应时要求该容器创建一个新会话，则抛出IllegalStateException。
     *
     * 参数：
     * create –如果需要，则为该请求创建一个新会话；为true； 如果没有当前会话，则返回false
     * 返回值：
     ...
     * @since Servlet 3.1
     * 更改与此请求关联的会话的会话ID。 此方法不会创建新的会话对象，而只会更改当前会话的ID。
     *
     * 返回值：
     * 分配给会话的新会话ID
     */
