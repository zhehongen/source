...
/**SessionRegistry的默认实现，它侦听在Spring应用程序上下文中发布的SessionDestroyedEvents。
 ...
 * <p>为了使此类在Web应用程序中正常运行，重要的是，您必须在web.xml文件中注册HttpSessionEventPublisher，以便此类通知过期的会话。
...
public class SessionRegistryImpl implements SessionRegistry,
	...
	private final ConcurrentMap<Object, Set<String>> principals;//principal->sessionIdSet
	...
	public void registerNewSession(String sessionId, Object principal) {
		...
		if (getSessionInformation(sessionId) != null) {//删除不会有并发问题，顶多发现已经被删除了。但是有没有可能查询的时候没有，往下执行的时候因为并发又有了？单项map似乎不影响，顶多就是覆盖
			removeSessionInformation(sessionId);
		}
