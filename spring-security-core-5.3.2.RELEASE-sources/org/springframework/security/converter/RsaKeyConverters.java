 ...
 * 用于创建java.security.Key转换器实例
 * @author Josh Cummings
 * @since 5.2
 */
public class RsaKeyConverters {
	private static final String DASHES = "-----";//长划
	...
	/**构造一个转换器，以将PEM编码的PKCS＃8 RSA私钥转换为RSAPrivateKey。
	 * Construct a {@link Converter} for converting a PEM-encoded PKCS#8 RSA Private Key
	 * into a {@link RSAPrivateKey}.请注意，密钥通常以PKCS＃1格式设置，并且可以通过标头轻松识别。
	 *如果密钥文件以“ ----- BEGIN RSA PRIVATE KEY -----”开头，则为PKCS＃1。 如果它是PKCS＃8格式的，则以“ ----- BEGIN PRIVATE KEY -----”开头。
	 ...
	 *该转换器不会关闭InputStream，以避免对流的起源和进一步使用做出不可移植的假设。
	 ...
	 *一个可以读取PEM编码的PKCS＃8 RSA私钥并返回RSAPrivateKey的转换器。
