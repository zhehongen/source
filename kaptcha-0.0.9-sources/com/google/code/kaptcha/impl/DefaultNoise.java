...
public class DefaultNoise extends Configurable implements NoiseProducer
{
			...
			float factorTwo, float factorThree, float factorFour)
	{
		...
		// the points where the line changes the stroke and direction线条改变笔画和方向的点
		Point2D[] pts = null;
		Random rand = new Random();

		// the curve from where the points are taken取点的曲线
		...
		// creates an iterator to define the boundary of the flattened curve创建一个迭代器来定义展平曲线的边界
		PathIterator pi = cc.getPathIterator(null, 2);
		Point2D tmp[] = new Point2D[200];
		int i = 0;

		// while pi is iterating the curve, adds points to tmp array当 pi 迭代曲线时，将点添加到 tmp 数组
		while (!pi.isDone())
		{
		...
		// for the maximum 3 point change the stroke and direction最多 3 点改变行程和方向
