...
public class DefaultWordRenderer extends Configurable implements WordRenderer
{
	...
	public BufferedImage renderWord(String word, int width, int height)
	{
		...
		char[] wordChars = word.toCharArray();//要渲染的几个字符
		Font[] chosenFonts = new Font[wordChars.length];//每个字符的字体
		int [] charWidths = new int[wordChars.length];//每个字符的宽度？
		int widthNeeded = 0;
		for (int i = 0; i < wordChars.length; i++)
		{
			chosenFonts[i] = fonts[random.nextInt(fonts.length)];//随机选择一种字体
