...
public class SecurityContextHolder {
	...
	/**
	 * Explicitly clears the context value from the current thread.从当前线程中明确清除上下文值。
	 */
	 ...
	 *主要用于故障排除，此方法显示该类已重新初始化其SecurityContextHolderStrategy的次数。
	 * @return the count (should be one unless you've called计数（除非您已调用setStrategyName（String）切换到其他策略，否则应为1）。
	 ...
	 *将新的SecurityContext与当前执行线程关联。
	 * @param context the new <code>SecurityContext</code> (may not be <code>null</code>)上下文–新的SecurityContext（不能为null）
	 */
	 ...
	 *更改首选策略。 对于给定的JVM，不要多次调用此方法，因为它将重新初始化该策略，并使用旧策略对现有的任何线程产生不利影响。
	 * @param strategyName the fully qualified class name of the strategy that should be
	 * used.strategyName-应该使用的策略的完全限定的类名。
	 */
 ...
 * 此类提供了一系列静态方法，这些方法委托给SecurityContextHolderStrategy的实例。该类的目的是提供一种方便的方法来指定应用于给定JVM的策略。这是JVM范围的设置，因为此类中的所有内容都是静态的，以便于调用代码。
 * 要指定应使用的策略，必须提供模式设置。模式设置是定义为静态最终字段的三个有效MODE_设置之一，或者是提供公共无参数构造函数的SecurityContextHolderStrategy的具体实现的完全限定的类名。
 * 有两种方法可以指定所需的策略模式字符串。首先是通过键入SYSTEM_PROPERTY的系统属性来指定它。第二种是在使用类之前调用​​setStrategyName（String）。如果两种方法都没有使用，则该类将默认使用MODE_THREADLOCAL，它向后兼容，JVM不兼容较少，并且适用于服务器（而MODE_GLOBAL绝对不适合服务器使用）。
 * */
