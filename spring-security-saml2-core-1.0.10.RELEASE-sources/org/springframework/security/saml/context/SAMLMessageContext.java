...
/**具有Spring Extension SAML模块特定值的消息上下文。
...
public class SAMLMessageContext extends BasicSAMLMessageContext {
    ...
    private Decrypter localDecrypter;//解密器
    ...
    private X509Credential peerSSLCredential;//我们的公钥
     ...
     *
     * @return trust engine used for verification of signatures coming from peers信任引擎，用于验证来自对等方的签名
     */
    ...
    /**证书使用了用于对我们的服务器进行身份验证的对等实体，作为SSL / TLS连接的一部分。 仅用于对等方发起的通信。
    ...
    /**确定对等实体是自动确定的（例如使用默认值）还是它是显式用户选择的结果。
