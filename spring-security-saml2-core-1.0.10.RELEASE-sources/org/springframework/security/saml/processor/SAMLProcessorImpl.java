...
/**处理器能够解析来自HttpServletRequest的SAML消息，并填充SAMLMessageContext以进行进一步的验证。
...
public class SAMLProcessorImpl implements SAMLProcessor {
    ...
    /**使用已配置的绑定之一加载传入的SAML消息，并使用它来填充SAMLMessageContext对象。该上下文应包含inboundMessageTransport和outboundMessageTransport。如果设置了localEntityId，localEntityRole或peerEntityRole，则将使用它，否则将默认SP加载为本地实体，将IDP假定为对等体。
     ...
     * @return SAML message context with filled information about the message具有有关消息的填充信息的SAML消息上下文
    ...
    /**填充要用于传入消息的安全策略，并将其在samlContext中设置为securityPolicyResolver。 使用所使用绑定的getSecurityPolicy方法填充SecurityPolicy。
    ...
    /**确定要用于给定端点的绑定。 默认情况下，使用从端点上的getBinding调用返回的绑定。 特殊处理用于（Holder of Key WebSSO）概要文件终结点，其中实际绑定存储在hoksso：ProtocolBinding属性下。
