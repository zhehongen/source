...
/**过滤器按照https://docs.oasis-open.org/security/saml/Post2.0/sstc-saml-idp-discovery.pdf中的定义实施身份提供者发现服务和配置文件。
...
public class SAMLDiscovery extends GenericFilterBean {
    ...
    /**发现服务用于发送响应的URL。 根据请求实体的元数据验证值。 URL可以包含其他查询部分，但不能包含与returnIdParam中指定的属性相同的属性。 IDP Disco规范的一部分。
    ...
    /**请求参数，指定用于传达确定的IDP名称的响应属性。 空时使用“ entityID”。 IDP Disco规范的一部分。
    ...
    protected void processDiscoveryRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String entityId = request.getParameter(ENTITY_ID_PARAM);//说明：entityID
        ...
        try {
            ...
            request.setAttribute(SAMLConstants.LOCAL_ENTITY_ID, entityId);//说明："localEntityId"
    ...
    protected void sendIDPSelection(HttpServletRequest request, HttpServletResponse response, String responseURL, String returnParam) throws IOException, ServletException {
        ...
        // Store the value
        request.setAttribute(RETURN_URL, responseURL);//说明："idpDiscoReturnURL"
        request.setAttribute(RETURN_PARAM, returnParam);//说明：idpDiscoReturnParam
    ...
    protected String getDefaultReturnURL(SAMLMessageContext messageContext) {
        ...
        if (extendedMetadata.isLocal() && extendedMetadata.getIdpDiscoveryResponseURL() != null) {
            return extendedMetadata.getIdpDiscoveryResponseURL();// /saml/login
        }
