 ...
 * 身份验证提供程序能够验证SAMLAuthenticationToken的有效性，并且在令牌有效的情况下也可以创建经过身份验证的UsernamePasswordAuthenticationToken。
...
public class SAMLAuthenticationProvider implements AuthenticationProvider, InitializingBean {
    ...
    /**如果为false（默认），则生成的Authentication对象将包括SAMLCredential的实例作为凭据值。 该凭证包括与身份验证过程有关的信息，接收到的属性，并且是“单次注销”所必需的。 如果您的应用程序不需要凭据，可以通过将此标志设置为true来将其从Authentication对象中排除。
