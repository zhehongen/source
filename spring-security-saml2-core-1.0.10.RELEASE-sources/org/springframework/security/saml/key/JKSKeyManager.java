...
/**类提供对SAML扩展配置的私钥和可信密钥的访问。 密钥存储在底层的KeyStore对象中。 该类还提供了用于加载证书和公共密钥的其他便捷方法。
...
public class JKSKeyManager implements KeyManager {
    ...
    /**使用现有KeyStore实例加载凭据的默认构造函数。 可用键是自动计算的。
     ...
     * @param passwords passwords used to access private keys用于访问私钥的密码
     ...
     * @param storePass password to access the keystore, or null for no password访问密钥库的密码，如果没有密码则为null
