 ...
 * 主题代表单个实体（例如人）的一组相关信息。此类信息包括主体的身份及其与安全性相关的属性（例如，密码和加密密钥）。
 * 主体可能具有多个身份。每个身份均表示为主体中的主体。
 * 委托人只是将名称绑定到主题。例如，一个主题恰好是一个人，爱丽丝，可能有两个委托人：
 * 一个将“ Alice Bar”（她的驾驶执照上的名称）绑定到该主题，
 * 另一个将“ 999-99-9999”绑定，她在学生证上的编号，发送给受试者。
 * 即使每个主体都具有不同的名称，两个主体也引用相同的主题。
 * 主体还可以拥有与安全性相关的属性，这些属性称为凭据。
 * 需要特殊保护的敏感凭据（例如私钥）存储在私有凭据集中。
 * 打算共享的凭证（例如公用密钥证书或Kerberos服务器票证）存储在公用凭证集中。
 * 访问和修改不同的凭据集需要不同的权限。
 * 要检索与主题关联的所有主体，请调用getPrincipals方法。
 * 要检索属于某个主题的所有公共或私有凭据，请分别调用getPublicCredentials方法或getPrivateCredentials方法。
 * 若要修改返回的主体集和凭据，请使用Set类中定义的方法。例如：
 *                 Subject subject;
 *                 Principal principal;
 *                 Object credential;
 *
 ...
 *                 subject.getPrincipals().add(principal);
 *
 * 该Subject类实现Serializable。与主题相关联的主体被序列化时，与主题相关联的凭证却没有。请注意，java.security.Principal类未实现Serializable。因此，与主题关联的所有具体Principal实现都必须实现Serializable。
 */
