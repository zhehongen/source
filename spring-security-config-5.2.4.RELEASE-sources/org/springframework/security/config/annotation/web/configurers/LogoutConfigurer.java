 ...
 *添加注销支持。 其他SecurityConfigurer实例可以在init（HttpSecurityBuilder）阶段调用addLogoutHandler（LogoutHandler）。
 * 安全过滤器
 * 下列过滤器已填充
 * 注销过滤器
 * 创建共享对象
 ...
 * 泛型嵌套泛型，不要关心内部泛型的参数，要只看内部的外壳。因为这只是一种编译时的东西。
 */
public final class LogoutConfigurer<H extends HttpSecurityBuilder<H>> extends
	 ...
	 * 添加一个LogoutHandler。 默认情况下，将SecurityContextLogoutHandler
	 * 和LogoutSuccessEventPublishingLogoutHandler添加为最后的LogoutHandler实例。
	 *
	 * 参数：
	 * logoutHandler –要添加的LogoutHandler
	 * 返回值：
	 * LogoutConfigurer进行进一步的自定义
	 */
	 ...
	 * @return the {@link LogoutConfigurer} for further customization
	 * 触发注销发生的URL（默认为“ / logout”）。如果启用了CSRF保护（默认），则请求也必须是POST。
	 * 这意味着默认情况下需要POST“ / logout”来触发注销。如果禁用CSRF保护，则允许使用任何HTTP方法。
	 * 最佳做法是在更改状态（即注销）的任何操作上使用HTTP POST来防御CSRF攻击。
	 * 如果您确实想使用HTTP GET，则可以使用logoutRequestMatcher（new AntPathRequestMatcher（logoutUrl，“ GET”））;
	 *
	 * 参数：
	 * logoutUrl –将调用注销的URL。
	 * 返回值：
