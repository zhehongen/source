...
public @interface RequestMapping {
	 ...
	 * 映射请求的参数，从而缩小了主映射的范围。
	 * 任何环境的格式均相同：“ myParam = myValue”样式表达式的序列，、
	 * 仅当发现每个这样的参数都具有给定值时才映射请求。可以使用“！=”运算符来否定表达式，
	 * 如“ myParam！= myValue”。还支持“ myParam”样式表达式，此类参数必须存在于请求中（允许具有任何值）。
	 * 最后，“！myParam”样式表达式指示指定的参数不应出现在请求中。
	 * 在类型级别和方法级别都受支持！当在类型级别使用时，所有方法级别的映射都继承此参数限制
	 * （即，甚至在解析处理程序方法之前都会检查类型级别的限制）。
	 * 参数映射被视为在类型级别上强制执行的限制。主路径映射（即指定的URI值）仍然必须唯一地标识目标处理程序，
	 * 而参数映射仅表示调用处理程序的前提条件。
	 */
