 ...
 * 将HttpSession实现切换为由Session支持。 SessionRepositoryFilter包装HttpServletRequest并重写方法，
 * 以获取由SessionRepository返回的Session支持的HttpSession。
 * SessionRepositoryFilter使用HttpSessionIdResolver（默认Cookie CookieHttpSessionIdResolver）
 * 在HttpSession和Session抽象之间架起逻辑桥梁。特别：
 * 使用HttpSessionIdResolver.resolveSessionIds（HttpServletRequest）查找会话ID。
 * 默认为查找名为SESSION的cookie。
 * 使用以下命令将新创建的会话的会话ID发送到客户端
 * 使用HttpSessionIdResolver.expireSession（HttpServletRequest，HttpServletResponse）
 * 通知客户端会话ID不再有效
 * 必须将SessionRepositoryFilter放置在访问HttpSession或可能提交响应的任何Filter之前，
 * 以确保该会话被覆盖并正确持久。
 * 以来：
 * 1.0
 * 类型参数：
...
public class SessionRepositoryFilter<S extends Session> extends OncePerRequestFilter {
	 ...
	 * @author Rob Winch
	 * @since 1.0
	 * 一个使用SessionRepository检索HttpSession的HttpServletRequest。
	 */
	private final class SessionRepositoryRequestWrapper extends HttpServletRequestWrapper {

		private final HttpServletResponse response;

		private S requestedSession;//1

		private boolean requestedSessionCached;//2

		private String requestedSessionId;//3一伙的
		 ...
		 * and persist the Session.使用HttpSessionIdResolver将会话ID写入响应并持久化Session。
		 */
		...
		public HttpSessionWrapper getSession(boolean create) {
			...
			if (requestedSession != null) {
				...
				// 这是无效的会话ID。 无需再次询问在此请求期间是否调用了request.getSession
