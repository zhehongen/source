...
public class DefaultCookieSerializer implements CookieSerializer {
	...
	private int getMaxAge(CookieValue cookieValue) {
		...
		if (maxAge < 0) {
			if (this.rememberMeRequestAttribute != null
				...
				// Cookie仅在创建会话时写入，因此如果启用了“记住我”功能，我们将依赖会话过期而不是Cookie过期
	 ...
	 * 设置是否应使用标记为仅HTTP的Cookie。 默认值为true。
	 *
	 * 参数：
	 * useHttpOnlyCookie –确定cookie是否应标记为“仅HTTP”。
	 */
	 ...
	 * 设置一个明确的域名。 当请求来自www.example.com时，这将允许使用“ example.com”的域。
	 * 这允许跨子域共享cookie。 默认为使用当前域。
	 *
	 * 参数：
	 * domainName –要使用的域的名称。 （即“ example.com”）
	 * 抛出：
	 * IllegalStateException-如果还设置了domainNamePattern
	 */
	 ...
	 * 设置不区分大小写的模式，该模式用于从HttpServletRequest.getServerName（）中提取域名。该模式应提供一个单独的分组，该分组定义应匹配的值。用户应注意不要输出诸如换行之类的恶意字符，以防止发生HTTP响应拆分之类的事情。
	 * 如果模式不匹配，则不会设置任何域。这对于确保在开发期间未使用本地主机时未设置域是很有用的。
	 * 一个示例值可能是"^.+?\\.(\\w+\\.[a-z]+)$"。对于给定的输入，它将提供以下显式域（null表示未设置域名）：
	 * example.com - null
	 ...
	 *
	 * 参数：
	 * domainNamePattern –不区分大小写的模式，用于提取域名
	 * 抛出：
	 * IllegalStateException-如果还设置了domainName
	 */
	 ...
	 * sessionId +“。 jvmRoute
	 * 要在每个JVM实例上使用set自定义路由，并设置一个前端代理，以根据该路由将所有请求转发到JVM。
	 *
	 * 参数：
	 * jvmRoute –要使用的JVM路由（即“ node01jvmA”，“ n01ja”等）
	 */
	 ...
	 * 设置是否应使用Cookie值的Base64编码。 为了支持建议将Base 64编码用于cookie值的RFC 6265，这很有价值。
	 *
	 * 参数：
	 * useBase64Encoding –该标志指示是否使用Base64编码
	 */
	 ...
	 * @since 1.3.0
	 * 设置指示“记住我”登录的请求属性名称。 如果指定，则cookie将写为Integer.MAX_VALUE。
	 *
	 * 参数：
	 * RememberMeRequestAttribute –记住我的请求属性名称
	 */
	 ...
	 * @since 2.1.0
	 * 设置SameSite cookie指令的值。 默认值为Lax。
	 *
	 * 参数：
	 * sameSite – SameSite指令值
	 */
