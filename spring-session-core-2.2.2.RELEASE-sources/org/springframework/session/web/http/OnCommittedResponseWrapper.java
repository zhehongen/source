 ...
 * @author Rob Winch
 * @since 1.0
 * 响应包装的基类，封装了在提交HttpServletResponse时处理事件的逻辑。
 */
abstract class OnCommittedResponseWrapper extends HttpServletResponseWrapper {
	 ...
	 * committed.
	 * Content-Length响应标头。 如果此值大于0，则一旦contentWritten大于或等于该响应，就将其视为已提交。
	 */
	 ...
	 * 提交HttpServletResponse时，调用此方法以禁用对onResponseCommitted（）的调用。
	 * 在发出异步Web请求的情况下，这很有用。
	 */
	 ...
	 * 只要未调用disableOnResponseCommitted（），就使用当前内容调用onResponseCommmitted（）。
	 */
