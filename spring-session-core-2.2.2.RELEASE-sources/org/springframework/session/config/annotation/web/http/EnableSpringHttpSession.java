 ...
 * @author Rob Winch
 * @since 1.1
 * 将此注释添加到@Configuration类，以将SessionRepositoryFilter公开为名为
 * “ springSessionRepositoryFilter”的bean，并由用户提供的SessionRepository实现支持。
 * 为了利用注释，必须提供一个SessionRepository bean。例如：
 * @Configuration
 ...
 *
 *    }
 *重要的是要注意，没有为您配置开箱即用的会话过期基础结构。
 * 这是因为会话期满之类的东西在很大程度上取决于实现。
 * 这意味着如果您需要清理过期的会话，则有责任清理过期的会话。
 * 为您提供了以下基本配置：
 * SessionRepositoryFilter-负责将HttpServletRequest
 * 与由SessionRepository支持的HttpSession实现包装在一起
 * SessionEventHttpSessionListenerAdapter-负责将
 * Spring Session事件转换为HttpSessionEvent。为了使其正常工作，
 * 您提供的SessionRepository的实现必须支持SessionCreatedEvent和SessionDestroyedEvent。
 */
