...
public abstract class AbstractDeclarable implements Declarable {
	 ...
	 * @param shouldDeclare true or false.此对象是否应由任何AmqpAdmin自动声明。 默认为true。
	 */
	 ...
	 * @param adminArgs The admins.应该声明该对象的AmqpAdmins； 默认是所有管理员。 空参数或带有单个空参数的array / varArg将清除集合（setAdminsThatShouldDeclare（（AmqpAdmin）空）或setAdminsThatShouldDeclare（（AmqpAdmin []）空）。 清除集合将重置行为，以便所有管理员将声明该对象。
	 */
