  ...
  public static CompositeFuture all(Future<?>... results) {
    ...
    for (int i = 0; i < len; i++) {
      results[i].setHandler(ar -> {
        ...
        if (ar.succeeded()) {
          synchronized (composite) {//这是什么意义？多个lamba会竞争
            composite.count++;
            if (!composite.isComplete() && composite.count == len) {
              handler = composite.setCompleted(null);//只有全部执行结束，才设置结束
            }
          }
        } else {
          synchronized (composite) {
            if (!composite.isComplete()) {
              handler = composite.setCompleted(ar.cause());//看不懂，到底想干啥？调用这个方法就是标记为已完成的
            }
          }
        }
        if (handler != null) {
    ...
    for (int i = 0;i < len;i++) {
      results[i].setHandler(ar -> {
        ...
        if (ar.succeeded()) {
          synchronized (composite) {
            if (!composite.isComplete()) {
              handler = composite.setCompleted(null);//只要有一个结束，就结束
            }
          }
        } else {
          synchronized (composite) {
