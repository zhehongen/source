 ...
 * @author Luke Taylor
 * @since 3.0
 * AuthenticationFailureHandler，当调用onAuthenticationFailure方法时，
 * 它将重定向到defaultFailureUrl属性的值。 如果尚未设置该属性，它将向客户端发送401响应，
 * 并包含导致失败的AuthenticationException的错误消息。
 * 如果设置了useForward属性，则将对目标而不是重定向进行RequestDispatcher.forward调用。
 */
public class SimpleUrlAuthenticationFailureHandler implements
	 ...
	 * 缓存AuthenticationException以用于视图渲染。
	 * 如果forwardToDestination设置为true，则将使用请求范围，否则它将尝试将异常存储在会话中。
	 * 如果没有会话且allowSessionCreation为true，则将创建一个会话。 否则，异常将不会被存储。
	 */
	 ...
	 * 如果设置为true，则执行对失败目标URL的转发而不是重定向。 默认为false
	 */
