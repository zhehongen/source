 ...
 * 一个用请求包装器填充ServletRequest的过滤器，该包装器实现Servlet API安全方法。
 * 扩展了SecurityContextHolderAwareRequestWrapper以提供以下其他方法：
 * HttpServletRequest.authenticate（HttpServletResponse）-允许用户确定他们是否已通过身份验证，
 * 以及是否未将用户发送到登录页面。请参见setAuthenticationEntryPoint（AuthenticationEntryPoint）。
 * HttpServletRequest.login（String，String）-允许用户使用AuthenticationManager进行身份验证。
 * 请参阅setAuthenticationManager（AuthenticationManager）。
 * HttpServletRequest.logout（）-允许用户使用Spring Security中配置的LogoutHandlers进行注销。
 * 请参见setLogoutHandlers（List）。
 * AsyncContext.start（Runnable）-将SecurityContext从调用AsyncContext.start（Runnable）
 * 的线程上找到的SecurityContextHolder自动复制到处理Runnable的线程中。
 */
