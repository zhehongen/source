 ...
 * @author Luke Taylor
 * @since 3.0
 * 一个SecurityContextRepository实现，将安全上下文存储在请求之间的HttpSession中。
 * 将查询HttpSession以在loadContext方法中检索SecurityContext（默认情况下使用键SPRING_SECURITY_CONTEXT_KEY）。
 * 如果出于任何原因都无法从HttpSession获得有效的SecurityContext，
 * 则将通过SecurityContextHolder.createEmptyContext（）调用来创建一个新的SecurityContext，并将返回该实例。
 * 调用saveContext时，上下文将存储在相同的键下，
 * 值已更改
 * 配置的AuthenticationTrustResolver不会报告内容代表匿名用户
 * 使用标准配置，如果loadContext不存在，则不会在HttpContext中创建任何HttpSession。
 * 当在Web请求的末尾调用saveContext且不存在会话时，仅在提供的SecurityContext不等于空的SecurityContext实例时，
 * 才会创建新的HttpSession。这样可以避免不必要的HttpSession创建，但是可以自动存储在请求期间对上下文所做的更改。
 * 请注意，如果将SecurityContextPersistenceFilter配置为热切创建会话，则此处应用的会话最小化逻辑不会有任何区别。
 * 如果使用急切的会话创建，则应确保将此类的allowSessionCreation属性设置为true（默认值）。
 * 如果出于任何原因都不应该创建HttpSession（例如，如果使用基本身份验证或永远不会显示相同jsessionid的类似客户端），
 * 则应将allowSessionCreation设置为false。
 * 仅在确实需要节省服务器内存并确保使用SecurityContextHolder的所有类被设计为在Web请求之间不保留SecurityContext时，
 * 才执行此操作。
 */
