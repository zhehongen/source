 ...
 * @author Luke Taylor
 * @since 3.0
 * 在请求之前，使用从配置的SecurityContextRepository获得的信息填充SecurityContextHolder，
 * 并在请求完成并清除上下文持有者后将其存储回存储库中。
 * 默认情况下，它使用HttpSessionSecurityContextRepository。请参阅此类以获取与HttpSession相关的配置选项的信息。
 * 该过滤器每个请求仅执行一次，以解决servlet容器（特别是Weblogic）的不兼容性。
 * 必须在任何身份验证处理机制之前执行此过滤器。
 * 身份验证处理机制（例如BASIC，CAS处理过滤器等）希望SecurityContextHolder在执行时包含有效的SecurityContext。
 * 这实质上是对旧HttpSessionContextIntegrationFilter的重构，以将存储问题委派给单独的策略，
 * 从而允许在请求之间维护安全上下文的方式进行更多自定义。
 * forceEagerSessionCreation属性可用于确保会话在过滤器链执行之前始终可用
 * （默认值为false，因为这会占用大量资源，因此不建议使用）。
 */
