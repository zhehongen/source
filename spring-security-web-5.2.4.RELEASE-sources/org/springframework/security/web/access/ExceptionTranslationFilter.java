 ...
 * 处理过滤器链中引发的所有AccessDeniedException和AuthenticationException。
 * 此过滤器是必需的，因为它提供了Java异常与HTTP响应之间的桥梁。它仅与维护用户界面有关。
 * 此筛选器不执行任何实际的安全性强制措施。
 * 如果检测到AuthenticationException，则筛选器将启动authenticationEntryPoint。
 * 这允许对源自org.springframework.security.access.intercept.AbstractSecurityInterceptor
 * 的任何子类的身份验证失败进行常见处理。
 * 如果检测到AccessDeniedException，则筛选器将确定该用户是否为匿名用户。
 * 如果他们是匿名用户，则将启动authenticationEntryPoint。
 * 如果他们不是匿名用户，则过滤器将委派给AccessDeniedHandler。默认情况下，筛选器将使用AccessDeniedHandlerImpl。
 * 要使用此过滤器，必须指定以下属性：
 * authenticationEntryPoint指示如果检测到AuthenticationException，则应开始身份验证过程的处理程序。
 * 请注意，这也可以将当前协议从http切换到https以进行SSL登录。
 * requestCache确定在身份验证过程中用于保存请求的策略，以便在用户身份验证后可以将其检索和重用。
 * 默认实现是HttpSessionRequestCache。
 */
