...
/**通过取消引用EncryptedData上下文的org.opensaml.xml.signature.KeyInfo的RetrievalMethod子级来查找EncryptedKey元素的EncryptedKeyResolver的实现。 RetrievalMethod必须具有Type属性，其值为EncryptionConstants.TYPE_ENCRYPTED_KEY。 URI属性值必须是同一文档的片段标识符（通过ID属性）。 此实现不支持处理RetrievalMethod的转换子级。
