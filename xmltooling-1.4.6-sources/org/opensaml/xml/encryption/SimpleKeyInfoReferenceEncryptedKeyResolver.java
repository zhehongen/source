...
/**实现EncryptedKeyResolver的方法是，通过取消引用EncryptedData上下文的KeyInfo的KeyInfoReference子代来查找EncryptedKey元素。 URI属性值必须是同一文档的片段标识符（通过ID属性）。 不支持处理外部资源。 此外，引用的目标本身必须包含EncryptedKey或后续的KeyInfoReference，且深度限制最大。 其他形式的解决方案不能与此混合在一起。
