...
public final class SecurityHelper {
    ...
    /**获取指定凭据的KeyInfoGenerator。返回的KeyInfoGenerator基于通过SecurityConfiguration.getKeyInfoGeneratorManager（）由指定安全配置定义的NamedKeyInfoGeneratorManager，并由签名凭据的类型和可选的KeyInfo生成器管理器名称确定。如果省略后者，则将使用安全配置的命名生成器管理器的默认管理器（NamedKeyInfoGeneratorManager.getDefaultManager（））。
     生成器由指定的SecurityConfiguration确定。如果未提供安全配置，则将使用全局安全配置（Configuration.getGlobalSecurityConfiguration（））。
