...
/**使用Criteria解析PKIXValidationInformation的解析器，通常将被基于PKIX的信任引擎使用。 实现也可以选择实现resolveTrustedNames（CriteriaSet），该方法将返回与标准所隐含的实体相关联的一组受信任的名称。 这些受信任的名称可用于（以特定于应用程序的方式）验证实体是否受信任可以持有特定证书。
