...
/**X509CredentialNameEvaluator的基本实现，它根据一组受信任的名称对从X509Credential的实体证书中提取的各种标识符进行评估。受支持的实体证书派生名称的类型用于名称检查：
 主题备用名称。主题专有名称中的第一个（即最特定的）公用名（CN）。完整的主题专有名称。默认情况下，所有受支持的名称类型均启用名称检查。使用X509Util中定义的适当常量值来指定要处理的使用者替代名称的类型。默认情况下，将检查以下类型的主题备用名称：DNS（X509Util.DNS_ALT_NAME）和URI（X509Util.URI_ALT_NAME）。将实体证书中的主题可分辨名称与受信任的密钥名称进行比较，以通过解析每个受信任项来实现完全DN匹配由配置的X500DNHandler实例返回的X500Principal中的密钥名称。然后，使用X500Principal.equals（Object）将所得的专有名称与证书主题进行比较。使用的默认X500DNHandler是InternalX500DNHandler。
