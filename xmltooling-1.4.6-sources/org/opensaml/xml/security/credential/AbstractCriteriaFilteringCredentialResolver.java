 ...
 */
public abstract class AbstractCriteriaFilteringCredentialResolver extends AbstractCredentialResolver {//看过了CriteriaFiltering
    ...
    private boolean unevaluableSatisfies;//看过了 不可估量的是否认为是满意的
    ...
    public Iterable<Credential> resolve(CriteriaSet criteriaSet) throws SecurityException {
        Iterable<Credential> storeCandidates = resolveFromSource(criteriaSet);//看过了,获取原始的所有凭据
        Set<EvaluableCriteria<Credential>> evaluableCriteria = getEvaluableCriteria(criteriaSet);//看过了。获取评估标准
        ...
        for (Criteria criteria : criteriaSet) {
            if (criteria instanceof EvaluableCredentialCriteria) {
                ...
                EvaluableCredentialCriteria evaluableCriteria =
                    EvaluableCredentialCriteriaRegistry.getEvaluator(criteria);//说明：从仓库拿
