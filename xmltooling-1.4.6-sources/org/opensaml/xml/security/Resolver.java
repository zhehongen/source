...
/**解析程序的通用接口，用于处理指定的标准并生成一些特定于实现的结果信息。
 * Generic interface for resolvers which process specified criteria and produce some implementation-specific
 * result information.
 *
 * @param <ProductType> the type of objects produced by this resolver此解析器生成的对象的类型
 * @param <CriteriaType> the type of criteria to process during resolution解析期间要处理的条件的类型
 */
