...
/**SignatureTrustEngine的一个实现，用于评估XML和原始签名的有效性和可信赖性。按BaseSignatureTrustEngine中的描述执行处理。 如果基于此处理，确定签名的KeyInfo不存在或不包含有效的（且受信任的）签名密钥，则信任引擎验证将失败。 由于PKIX引擎基于事先不知道受信任的签名密钥的假设，因此签名密钥必须存在于签名的KeyInfo元素中，或者可以从签名的KeyInfo元素中的信息派生。
