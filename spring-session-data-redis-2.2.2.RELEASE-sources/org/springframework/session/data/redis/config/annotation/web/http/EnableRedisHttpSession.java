 ...
 * 将此注释添加到@Configuration类，以将SessionRepositoryFilter
 * 公开为一个名为springSessionRepositoryFilter并由Redis支持的bean。
 * 为了利用注释，必须提供单个RedisConnectionFactory。 例如：
 *    @Configuration
 *    @EnableRedisHttpSession
 *    public class RedisHttpSessionConfig {
 *
 ...
 *
 *    }
 * 更高级的配置可以扩展RedisHttpSessionConfiguration。
 */
...
public @interface EnableRedisHttpSession {
	 ...
	 * 会话超时（以秒为单位）。 默认情况下，它设置为1800秒（30分钟）。 这应该是一个非负整数。
	 *
	 * 返回值：
	 * 会话在过期前可以处于无效状态的秒数
	 */
