 ...
 * @author Rob Winch
 * @since 1.0
 * RedisIndexedSessionRepository.RedisSession实例到期的策略。
 * 这执行两个操作：Redis无法保证何时触发过期的会话事件。
 * 为了确保及时处理过期的会话事件，将过期（四舍五入到最近的分钟）映射到当时所有过期的会话。
 * 每当调用cleanExpiredSessions（）时，就会访问前一分钟的会话，以确保它们在过期时被删除。
 * 在某些情况下，cleanExpiredSessions（）方法可能在特定时间内没有被调用。
 * 例如，重新启动服务器时可能会发生这种情况。 为了解决这个问题，还设置了Redis会话的到期时间。
 */
final class RedisSessionExpirationPolicy {
	...
	private final RedisOperations<Object, Object> redis;//redistemplate

	private final Function<Long, String> lookupExpirationKey; //查找过期key
