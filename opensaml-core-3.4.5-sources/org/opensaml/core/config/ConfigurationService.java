...
/**提供与库模块配置有关的对象的注册，检索和注销的服务。该服务使用Configuration的内部管理实例来处理其管理下的配置对象的注册，检索和注销
 ...
 *该服务首先尝试使用Java Services API来解析要使用的Configuration实例。如果通过Services API机制注册了Configuration的多个实现，则将使用ServiceLoader迭代器返回的第一个实现。如果没有使用Service API声明或解析任何Configuration实施，则它将使用默认的实施MapBasedConfiguration。
 * <p>
 ...
 * management.
 * </p>
 *也可以通过setConfiguration（Configuration）在外部设置要使用的Configuration实例。在使用特定于应用程序的方式（例如Spring）来配置环境时，这可能很有用。这将覆盖上述解决过程。
 * <p>
